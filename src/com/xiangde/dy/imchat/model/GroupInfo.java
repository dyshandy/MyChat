...
import com.xiangde.dy.imchat.MyApplication;
import com.xiangde.dy.imchat.ui.GroupProfileActivity;
...
public class GroupInfo implements Observer {
	...
	public static final String publicGroup = "Public",
			privateGroup = "Private", chatRoom = "ChatRoom";

	private GroupInfo() {
		groups = new HashMap<String, List<GroupProfile>>();
	...
	private void refresh() {
		List<TIMGroupCacheInfo> groupInfos = TIMGroupAssistant.getInstance()
				.getGroups(null);
		if (groupInfos == null)
			return;
		for (TIMGroupCacheInfo item : groupInfos) {
			List<GroupProfile> list = groups.get(item.getGroupInfo()
					.getGroupType());
			if (list == null)
				continue;
	...
	private void updateGroup(TIMGroupCacheInfo info) {
		if (groups == null
				|| groups.get(info.getGroupInfo().getGroupType()) == null)
			return;
		...
		groups.get(info.getGroupInfo().getGroupType()).add(
				new GroupProfile(info));
	}
	...
	public List<ProfileSummary> getGroupListByType(String type) {
		List<ProfileSummary> result = new ArrayList<ProfileSummary>();
