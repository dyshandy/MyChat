...
public class ChatInput extends RelativeLayout implements TextWatcher,
	...
	private int coumunts=35;

	private DotViewPager dotviewPager;
	...
	private void initView() {
		...
		dotviewPager = (DotViewPager) findViewById(R.id.my_viewpager);
	}
	...
	private void prepareEmoticon() {
		if (emoticonPanel == null)
			return;
		for (int k = 0; k < getAssetsSize(); k++) {
			System.out.println("-------K:" + k);

			LinearLayout viewcontent = new LinearLayout(getContext());
			viewcontent.setLayoutParams(new LinearLayout.LayoutParams(
			...
			for (int i = 0; i < 5; ++i) {
				...
				for (int j = 0; j < 7; ++j) {
					try {
						...
						final int index = coumunts * k + 7 * i + j;
						System.out.println("---------index:" + index);
						/*通过文件名称获取文件流*/
						InputStream is = am.open(String.format(
								"emoticon/%d.gif", index));
				...
				viewcontent.addView(linearLayout);
			}
			views .add(viewcontent);
		}
		/* 图片完全加载后才准备好 */
		isEmoticonReady = true;
		dotviewPager.setViewPagerViews(getContext(), views);
	}
	...
	/* 获取assets/emoticon文件夹下图标数量 */
	public int getAssetsSize() {
		String[] llist_image = null;
		int rows = 0;
		try {
			llist_image = getContext().getAssets().list("emoticon");

		} catch (IOException e) {
			e.printStackTrace();
		}
		System.out.println("图标个数："+llist_image.length);
		// return llist_image.length;
		/* 有多少个viewpager */
		rows = llist_image.length / coumunts;

		if (llist_image.length % coumunts > 0) {
			rows = rows + 1;
		}
		return rows;
	}
